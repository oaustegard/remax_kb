"""Skill entry point: search a .kb (v1) or .kbi (v2) from the command line.

Caches the opened KB and the loaded embedder in module-level state so
repeated invocations within one Python process (e.g. multiple tool
calls in the same session) reuse them. The cache is keyed on the
artifact path's resolved string, so switching files is supported.

Format is auto-detected:

* v1 ``.kb`` — single-zip, dense Hamming top-k.
* v2 ``.kbi`` (+ sibling ``.kbc/``) — split index, hybrid (dense +
  BM25) retrieval with RRF fusion, lazy chunk fetch with sha256 verify.

Usage:
    python skill/search.py --kb /mnt/user-data/uploads/foo.kb --query "..." --k 5
    python skill/search.py --kb /mnt/user-data/uploads/foo.kbi --query "..." --k 5

Output: JSON list of hits. v1 hits carry ``distance`` (Hamming); v2 hits
carry ``fused``/``dense_distance``/``bm25_score``/``verified``. Both
carry ``id``, ``text``, ``meta``.
"""
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

# Allow running from a checkout without `pip install -e .`.
_REPO_ROOT = Path(__file__).resolve().parents[1]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from remax_kb import KB, KBv2, detect_format  # noqa: E402
from remax_kb.embedders import JinaONNXEmbedder  # noqa: E402


_kb_cache: dict[str, object] = {}
_embedder: JinaONNXEmbedder | None = None


def _get_embedder() -> JinaONNXEmbedder:
    global _embedder
    if _embedder is None:
        _embedder = JinaONNXEmbedder()
    return _embedder


def _get_kb(path: str):
    key = str(Path(path).resolve())
    if key not in _kb_cache:
        fmt = detect_format(key)
        _kb_cache[key] = KBv2.open(key) if fmt == "2" else KB.open(key)
    return _kb_cache[key]


def search(kb_path: str, query: str, k: int = 5, *, alpha: float | None = None) -> list[dict]:
    kb = _get_kb(kb_path)
    emb = _get_embedder()

    if isinstance(kb, KBv2):
        hits = kb.search_and_fetch(query, embedder=emb, k=k, alpha=alpha)
        return [
            {
                "id": h.chunk_id,
                "fused": h.fused,
                "dense_distance": h.dense_dist,
                "bm25_score": h.bm25_score,
                "verified": h.verified,
                "text": h.text,
                "meta": h.meta,
            }
            for h in hits
        ]

    hits = kb.search(query, embedder=emb, k=k)
    return [
        {
            "id": chunk["id"],
            "distance": int(dist),
            "text": chunk["text"],
            "meta": chunk.get("meta", {}),
        }
        for dist, chunk in hits
    ]


def _resolve_default_kb() -> str | None:
    """Implement the resolution chain from SKILL.md (v1 .kb and v2 .kbi)."""
    candidates: list[Path] = []
    for root in ("/mnt/user-data/uploads", "/mnt/project"):
        p = Path(root)
        if p.exists():
            candidates.extend(p.glob("*.kb"))
            candidates.extend(p.glob("*.kbi"))
    if not candidates:
        return None
    candidates.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    return str(candidates[0])


def _main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--kb", help="path to .kb (v1) or .kbi (v2) (default: auto-resolve)")
    ap.add_argument("--query", required=True, help="user query string")
    ap.add_argument("--k", type=int, default=5, help="number of results")
    ap.add_argument(
        "--alpha",
        type=float,
        default=None,
        help="(v2 only) fusion weight; omit for RRF, set 0..1 for weighted",
    )
    ap.add_argument(
        "--pretty",
        action="store_true",
        help="indent the JSON output for human reading",
    )
    args = ap.parse_args()

    kb_path = args.kb or _resolve_default_kb()
    if not kb_path:
        print(
            "No --kb given and no .kb/.kbi files found under "
            "/mnt/user-data/uploads or /mnt/project. Pass --kb explicitly.",
            file=sys.stderr,
        )
        return 2

    hits = search(kb_path, args.query, k=args.k, alpha=args.alpha)
    indent = 2 if args.pretty else None
    print(json.dumps({"kb": kb_path, "query": args.query, "hits": hits}, indent=indent, ensure_ascii=False))
    return 0


if __name__ == "__main__":
    raise SystemExit(_main())
